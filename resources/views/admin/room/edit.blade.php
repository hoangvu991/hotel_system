    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            ...
            <h6 class="m-0 font-weight-bold text-primary">
                <a href="{{ url('admin/room/'.$edit->id) }}" class="float-right btn btn-success btn-sm">View All</a>
            </h6>
        </div>
        <div class="card-body">
                ...
                <div class="mb-3">
                  ...
                  <select name="room_type_id" id="" class="form-control">
                    ...
                    @foreach ($roomType as $item)
                        <option {{ $edit->room_type_id ===  $item->id ? 'selected' : '' }}  value="{{ $item->id }}">{{ $item->title }}</option>           
                    @endforeach
                  </select>
                </div>
